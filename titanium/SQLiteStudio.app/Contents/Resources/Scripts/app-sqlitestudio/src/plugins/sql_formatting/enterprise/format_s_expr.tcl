...
class FormatStatementExprSuffix {
	inherit FormatStatement
...
class FormatStatementRaiseFunction {
	inherit FormatStatement
...
body FormatStatementExprOnly::formatSql {} {
	set sql ""
	switch -- [get branchIndex] {
		...
		14 {
			...
			if {[get existsKeyword]} {
				append sql [case "EXISTS"]
			}
...
body FormatStatementExprSuffix::formatSql {} {
...
# 			if {$orAnd} {
# 				copyMarkTo $exprFmt exprOnlyBegin
# 			}
#
			set exprSql [$exprFmt formatSql]
...
body FormatStatementRaiseFunction::formatSql {} {
	set sql [case "RAISE"]
	handleLeftParExpr sql
