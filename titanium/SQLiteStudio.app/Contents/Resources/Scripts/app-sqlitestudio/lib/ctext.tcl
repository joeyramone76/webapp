...
#
# Little modified by Pawe³ Salawa for SQLiteStudio needs.
#
...
proc ctext::buildArgParseTable win {
	...
	lappend argTable {0 false no} -readonly {
		set configAr(-readonly) 0
		break
	}

	lappend argTable {1 true yes} -readonly {
		set configAr(-readonly) 1
		break
	}
...
proc ctext::addHighlightClassForSpecialChars {win class color chars} {
	set charList [split [string toupper $chars] ""]
...
proc ctext::highlight {win start end} {
	...
	while 1 {
		set res [$twin search -count length -regexp -nocase -- {([^\s\(\{\[\}\]\)\.\t\n\r;\"'\|,]+)} $si $end]
		if {$res == ""} {
			break
		}

		set wordEnd [$twin index "$res + $length chars"]
		set word [string toupper [$twin get $res $wordEnd]]
