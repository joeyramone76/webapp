	...
	proc xview {w {cmd ""} args} {
		...
		if {$vleft != $_vleft || $count == 0} {
			...
			if {$($w:fillx) && ($_vwidth < $_width || $($w:xscroll) == "") } {
				...
				place $w.scrolled -in $w -x [expr {-int($vleft)}] -width $_width
				if {$(debug,place)} { puts "place $w.scrolled -in $w -x [expr {-int($vleft)}] -width $_width" } ;#DEBUG
			} else {
				place $w.scrolled -in $w -x [expr {-int($vleft)}] -width {}
				if {$(debug,place)} { puts "place $w.scrolled -in $w -x [expr {-int($vleft)}] -width {}" } ;#DEBUG
			}
		}
		if {$($w:xscrollcallback) != ""} {
			uplevel #0 $($w:xscrollcallback)
		}
	}
	...
	proc yview {w {cmd ""} args} {
		...
		if {$vtop != $_vtop || $count == 0} {
			...
			if {$($w:filly) && ($_vheight < $_height || $($w:yscroll) == "")} {
				...
				place $w.scrolled -in $w -y [expr {-int($vtop)}] -height $_height
				if {$(debug,place)} {
					puts "place $w.scrolled -in $w -y [expr {-int($vtop)}] -height $_height"
				} ;#DEBUG
			} else {
				place $w.scrolled -in $w -y [expr {-int($vtop)}] -height {}
				if {$(debug,place)} {
					puts "place $w.scrolled -in $w -y [expr {-int($vtop)}] -height {}"
				} ;#DEBUG
			}
		}
	}
