...
//Rare exceptions to indicate a bug in the _shenglong_electricv code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_shenglong_electricv = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_shenglong_electricv]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.shenglong_electricv.module"
