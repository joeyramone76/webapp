...
NSString * shenglong_electricv$ModuleRequireFormat = @"(function(exports){"
...
@implementation shenglong_electricvObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_shenglong_electricv);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create shenglong_electricv global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "shenglong_electricv" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_shenglong_electricv = [[shenglong_electricvObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_shenglong_electricv];
	
	NSString *_shenglong_electricvNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _shenglong_electricvNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_shenglong_electricv valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:shenglong_electricv$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.shenglong_electricv.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.shenglong_electricv.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
